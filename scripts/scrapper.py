...
def _soup_tropicopia_house_plant(html_content):
    ...
    for tag in abstract:
        ...
        if key_bool:  # ako je key na redu
            # svaki key ima zadnja dva chara " :"
            key = cleared_tag[:-2].replace(" ", "_")
        else:
            # provjeravamo da li postoji value. ako ima ':' onda je key, pogledaj predhodni komentar
            if cleared_tag.find(":") > 0:
                # spremi za iduci key, ali nemoj ga gledat. sto znaci da je iduci cleared_tag moguci value
                key = cleared_tag[:-2].replace(" ", "_")
                cleared_tag = "None"
